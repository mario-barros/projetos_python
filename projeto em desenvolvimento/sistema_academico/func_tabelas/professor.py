...
caminho_arquivo = os.path.join(diretorio_actual, '../banco_dados', 'professor.db')

fg.acessar_banco(caminho_arquivo)
#Função para criar tabelas dos professores
def tabela_professor():
    banco, cursor = fg.acessar_banco(caminho_arquivo)
    try:
        cursor.execute("CREATE TABLE IF NOT EXISTS professor(id INTEGER PRIMARY KEY, nome TEXT NOT NULL, especialidade TEXT NOT NULL, localidade TEXT NOT NULL)")
        banco.commit()
